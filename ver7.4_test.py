...
# Simple letter sampling mode: when True, use a column-scan or flat baseline
# styles: 'ridge' -> top-most pixel per column; 'flat' -> horizontal baseline
SIMPLE_LETTER_MODE = True
SIMPLE_LETTER_STYLE = 'ridge'  # 'ridge' or 'flat'
...
def get_button_rects(w, h):
    ...
    return {
        ...
        "wave_bars":  pygame.Rect(col0, row(7), bw, bh),
            "show_video": pygame.Rect(col0, row(8), bw, bh),
        "phonk":      pygame.Rect(col1, row(0), bw, bh),
        "bg_load":    pygame.Rect(col1, row(1), bw, bh),
        "genre":      pygame.Rect(col1, row(2), bw, bh),
        "rgb":        pygame.Rect(col1, row(3), bw, bh),
        "letter_stable": pygame.Rect(col1, row(4), bw, bh),
...
letter_outer_points = []  # points for outer contours only (for ghost fill)
...
_trail_surf = None  # reused for RGB trail layers

# --- RGB GLITCH / TRAIL ---
letter_trail_mode = False
_glitch_offset = np.array([0.0, 0.0])   # current chromatic offset (lerps to target)
_glitch_target  = np.array([0.0, 0.0])  # snaps on beat, decays fast
# letter variant: 'test' (from working) or 'stable' (from ver7.2)
letter_variant = 'test'
...
def generate_letter_points_stable(char):
    """Stable, simpler generator copied from ver7.2: sample mask outline evenly."""
    global letter_points, letter_normals, letter_centroid, _ghost_surf, letter_outer_points
...
def generate_letter_points(char):
    """
    Generate SAMPLE_COUNT points around all contours (outer + holes) of the glyph.
    Simpler, robust version taken from the working branch.
    """
    global letter_points, letter_normals, letter_centroid, _ghost_surf, letter_outer_points, letter_variant
    # if stable variant requested, defer to stable implementation
    if letter_variant == 'stable':
        return generate_letter_points_stable(char)
    size = int(min(WIDTH, HEIGHT) * 0.68)
    font = get_font(size, bold=True)
    ...
    # Build filled boolean array: filled[y, x]
    filled = np.zeros((h, w), dtype=bool)
    for yy in range(h):
        for xx in range(w):
            filled[yy, xx] = bool(mask.get_at((xx, yy)))

    # Flood-fill from borders to mark outside background
    ...
    # Find filled connected components (outer strokes)
    filled_labels = np.zeros_like(filled, dtype=int)
    comps = []
    label = 1
    for yy in range(h):
        for xx in range(w):
            if filled[yy, xx] and filled_labels[yy, xx] == 0:
                stack = [(xx, yy)]
                filled_labels[yy, xx] = label
                comp = [(xx, yy)]
                while stack:
                    x, y = stack.pop()
                    for nx, ny in ((x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)):
                        if 0 <= nx < w and 0 <= ny < h and filled[ny, nx] and filled_labels[ny, nx] == 0:
                            filled_labels[ny, nx] = label
                            stack.append((nx, ny))
                            comp.append((nx, ny))
                comps.append(comp)
                label += 1

    # Find hole components (background regions not connected to outside)
    hole_labels = np.zeros_like(filled, dtype=int)
    holes = []
    hole_label = 1
    for yy in range(h):
        for xx in range(w):
            if (not filled[yy, xx]) and (not outside[yy, xx]) and hole_labels[yy, xx] == 0:
                stack = [(xx, yy)]
                hole_labels[yy, xx] = hole_label
                comp = [(xx, yy)]
                while stack:
                    x, y = stack.pop()
                    for nx, ny in ((x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)):
                        if 0 <= nx < w and 0 <= ny < h and (not filled[ny, nx]) and hole_labels[ny, nx] == 0 and (not outside[ny, nx]):
                            hole_labels[ny, nx] = hole_label
                            stack.append((nx, ny))
                            comp.append((nx, ny))
                holes.append(comp)
                hole_label += 1

    # Build ordered boundary points for each filled component (outer contours)
    contours = []
    def boundary_of_component(pixel_coords, interior=True):
        s = set(pixel_coords)
        b = []
        ...
        if not b:
            return []
        cx = sum(x for x, y in b) / len(b)
        cy = sum(y for x, y in b) / len(b)
        b.sort(key=lambda p: math.atan2(p[1] - cy, p[0] - cx))
        return b, (cx, cy)

    for comp in comps:
        b_res = boundary_of_component(comp, interior=True)
        if b_res:
            b, cent = b_res
            contours.append({'points': b, 'is_hole': False, 'centroid': cent})
    ...
    for hole in holes:
        ...
        cy = sum(y for x, y in b) / len(b)
        b.sort(key=lambda p: math.atan2(p[1] - cy, p[0] - cx))
        contours.append({'points': b, 'is_hole': True, 'centroid': (cx, cy)})
    ...
    outer_pts_flat = [p for c in contours if not c['is_hole'] for p in c['points']]
    if outer_pts_flat:
        letter_centroid = (
            sum(x for x, y in outer_pts_flat) / len(outer_pts_flat) + (WIDTH // 2 - w // 2),
            sum(y for x, y in outer_pts_flat) / len(outer_pts_flat) + (HEIGHT // 2 - h // 2),
        )
    else:
        letter_centroid = (WIDTH // 2, HEIGHT // 2)

    perimeters = [len(c['points']) for c in contours]
    ...
    counts = []
    acc = 0
    for i, p_len in enumerate(perimeters):
        if i == len(perimeters) - 1:
            cnt = SAMPLE_COUNT - acc
        else:
            cnt = max(4, int(round(SAMPLE_COUNT * (p_len / total_perim))))
            acc += cnt
        counts.append(cnt)
    ...
    outer_only = []
    rect = surf.get_rect(center=(WIDTH // 2, HEIGHT // 2))
    for c, cnt in zip(contours, counts):
        pts = c['points']
        if not pts:
            continue
        L = len(pts)
        sampled = [pts[int(i * L / cnt) % L] for i in range(cnt)]
        sampled_screen = [(x + rect.x, y + rect.y) for x, y in sampled]
        if c['is_hole']:
            cent = (c['centroid'][0] + rect.x, c['centroid'][1] + rect.y)
            nrm = _compute_letter_normals(sampled_screen, cent)
            nrm = [(-nx, -ny) for nx, ny in nrm]
        else:
            cent_overall = letter_centroid
            nrm = _compute_letter_normals(sampled_screen, cent_overall)
        pts_out.extend(sampled_screen)
        norms_out.extend(nrm)
        if not c['is_hole']:
            outer_only.extend(sampled_screen)

    letter_points = pts_out
    letter_normals = norms_out
    letter_outer_points = outer_only
    _ghost_surf = None
...
def draw_letter_visualizer_genre(screen, final_bars, shake, glow_surf, active_genres_set, avee_on, peak_arr, letter_pts, letter_norms, letter_cent):
    ...
    # Use only outer contour points for ghost fill to avoid mixing holes
    outer = letter_outer_points if 'letter_outer_points' in globals() and len(letter_outer_points) > 2 else letter_pts
    if len(outer) > 2:
        ...
        pygame.draw.polygon(_ghost_surf, (255, 50, 200, 18), outer)
        screen.blit(_ghost_surf, (0, 0))

    sx, sy = shake[0], shake[1]
    n = len(letter_pts)
    for i in range(n):
        intensity = final_bars[i % len(final_bars)]
        h = intensity * max_h
        px, py = letter_pts[i]
...
def _draw_letter_bars_to(surface, final_bars, sx, sy, color_override=None, alpha=255, width=4):
    """Helper: draw letter bars onto any surface with an optional offset and color."""
    max_h = min(WIDTH, HEIGHT) * 0.20
    ...
    for i in range(n):
        ...
        if color_override:
            ...
            col = bar_color_phonk(i, SAMPLE_COUNT, intensity)
        else:
            col = bar_color_letter(i, SAMPLE_COUNT, intensity)
        pygame.draw.line(surface, col, (int(x1), int(y1)), (int(ox), int(oy)), width)


def draw_letter_visualizer_enhanced(screen, final_bars, shake, glow_surf, on_beat=False):
    """
    Draw bars along letter outline with outward normals.
    RGB glitch: on beat, chromatic-aberration layers snap to large offsets then
    spring back quickly — giving a swift, punchy glitch look.
    """
    global _ghost_surf, _trail_surf, _glitch_offset, _glitch_target
    if not letter_points:
        return
    max_h = min(WIDTH, HEIGHT) * 0.20

    # --- Update glitch offset ---
    if letter_trail_mode:
        if on_beat:
            # Snap to a large random offset instantly
            strength = 18 + beat_confidence * 28
            angle = random.uniform(0, 2 * math.pi)
            _glitch_target[0] = math.cos(angle) * strength
            _glitch_target[1] = math.sin(angle) * strength
        # Fast spring-back: lerp offset toward target, then target decays hard
        _glitch_offset[:] = _glitch_offset * 0.35 + _glitch_target * 0.65
        _glitch_target[:] *= 0.30   # target collapses very fast → swift snap-back

    # Ghost fill
    # Use only outer contour points for ghost fill to avoid mixing holes
    outer = letter_outer_points if 'letter_outer_points' in globals() and len(letter_outer_points) > 2 else letter_points
    if len(outer) > 2:
        if _ghost_surf is None or _ghost_surf.get_size() != (WIDTH, HEIGHT):
            _ghost_surf = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
        _ghost_surf.fill((0, 0, 0, 0))
        ghost_color = (255, 50, 200, 18) if phonk_mode else (100, 220, 255, 18)
        pygame.draw.polygon(_ghost_surf, ghost_color, outer)
        screen.blit(_ghost_surf, (0, 0))

    sx, sy = shake[0], shake[1]

    # --- RGB glitch chromatic aberration layers ---
    if letter_trail_mode and (abs(_glitch_offset[0]) > 0.5 or abs(_glitch_offset[1]) > 0.5):
        if _trail_surf is None or _trail_surf.get_size() != (WIDTH, HEIGHT):
            _trail_surf = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)

        ox_g, oy_g = _glitch_offset[0], _glitch_offset[1]

        # Three chromatic channels — R displaced opposite, B displaced along offset
        layers = [
            ((255, 0,   40),  -ox_g * 1.0, -oy_g * 1.0, 110),   # red:  opposite direction
            ((0,   255, 80),   ox_g * 0.4,  oy_g * 0.4,  80),    # green: slight follow
            ((30,  80,  255),  ox_g * 1.0,  oy_g * 1.0, 110),    # blue: full offset
        ]
        for col, dx, dy, layer_alpha in layers:
            _trail_surf.fill((0, 0, 0, 0))
            _draw_letter_bars_to(_trail_surf, final_bars, sx + dx, sy + dy,
                                 color_override=col, width=4)
            _trail_surf.set_alpha(layer_alpha)
            screen.blit(_trail_surf, (0, 0), special_flags=pygame.BLEND_ADD)

    # --- Main bars (normal color, on top) ---
    if not letter_points:
        return
    n = len(letter_points)
    for i in range(n):
        ...
        r, g, b = color
        pygame.draw.line(glow_surf, (r, g, b, 45), (x1, y1), (ox2, oy2), 9)
        pygame.draw.line(screen, color, (x1, y1), (ox2, oy2), 4)
        pygame.draw.line(screen, color, (x1, y1), (ix2, iy2), 2)
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and show_ui:
            if BUTTON_RECTS["genre"].collidepoint(event.pos):
                ...
                p = filedialog.askopenfilename(filetypes=[("Audio/Video", "*.mp3 *.wav *.flac *.ogg *.mp4 *.m4v *.mov *.mkv *.avi *.webm")])
                if p:
                    _, ext = os.path.splitext(p)
                    ext = ext.lower()
                    video_exts = ('.mp4', '.m4v', '.mov', '.mkv', '.avi', '.webm')
                    # If it's a video and moviepy is available, load clip and extract audio
                    if ext in video_exts and mpy is not None:
                        try:
                            # close previous clip if any
                            if video_clip is not None:
                                try:
                                    video_clip.close()
                                except Exception:
                                    pass
                            video_clip = mpy.VideoFileClip(p)
                            video_loaded = True
                            video_show = False
                            # extract audio to temp wav for playback
                            if video_clip.audio is not None:
                                tf = tempfile.NamedTemporaryFile(delete=False, suffix='.wav')
                                video_temp_audio = tf.name
                                tf.close()
                                video_clip.audio.write_audiofile(video_temp_audio, fps=44100, verbose=False, logger=None)
                                reload_audio(video_temp_audio)
                            else:
                                # no audio; just mark loaded
                                data = None
                        except Exception:
                            video_clip = None
                            video_loaded = False
                ...
                # Toggle test-letter mode
                if letter_mode and letter_variant == 'test':
                    letter_mode = False
                else:
                    letter_mode = True
                    letter_variant = 'test'
                    generate_letter_points(title[0] if title else "A")
            elif BUTTON_RECTS["letter_stable"].collidepoint(event.pos):
                # Toggle stable-letter mode
                if letter_mode and letter_variant == 'stable':
                    letter_mode = False
                else:
                    letter_mode = True
                    letter_variant = 'stable'
                    generate_letter_points(title[0] if title else "A")
            ...
            elif BUTTON_RECTS["show_video"].collidepoint(event.pos):
                # Toggle showing the loaded video (video stays loaded and audio plays regardless)
                if video_loaded:
                    video_show = not video_show
    ...
    # ---- Background ----
    if video_loaded and video_show and video_clip is not None:
        try:
            t_video = time.time() - start_time
            dur = getattr(video_clip, 'duration', None)
            if dur and dur > 0:
                t_get = t_video % dur
            else:
                t_get = t_video
            frame = video_clip.get_frame(t_get)
            # frame is HxWx3 uint8 RGB
            h, w = frame.shape[0], frame.shape[1]
            fb = frame.tobytes()
            frame_surf = pygame.image.frombuffer(fb, (w, h), 'RGB')
            frame_surf = pygame.transform.smoothscale(frame_surf, (WIDTH, HEIGHT))
            screen.blit(frame_surf, (0, 0))
            # dim the video so visualizer remains visible
    ...
    if data is not None:
        ...
        if idx + 2048 < len(data):
            ...
            if letter_mode:
                draw_letter_visualizer_enhanced(screen, final_bars, shake, glow_surf, False)
            else:
                draw_circle_visualizer_enhanced(screen, final_bars, shake, glow_surf)
            screen.blit(glow_surf, (0, 0))
            if not letter_mode:
    ...
    if show_ui:
        label_map = {
            ...
            "letter":     "LETTER(TEST) ✓"     if (letter_mode and letter_variant=='test')     else "LETTER(TEST)",
            "letter_stable": "LETTER(STABLE) ✓" if (letter_mode and letter_variant=='stable') else "LETTER(STABLE)",
            ...
            "genre":      "GENRE",
            "rgb":        "RGB ✓"        if letter_trail_mode else "RGB",
            "show_video": "SHOW VIDEO ✓" if video_show else "SHOW VIDEO",
        }
        for k, r in BUTTON_RECTS.items():
            if k == "phonk":
                ...
                active = (letter_mode and letter_variant == 'test')
            elif k == "letter_stable":
                active = (letter_mode and letter_variant == 'stable')
            else:
                active = (
            ...
            if k == "rgb":
                txt_color = (200, 100, 255) if active else (160, 130, 200)
