...
def generate_letter_points(char):
    ...
    Strategy:
    - rasterize glyph into a mask
    - build a boolean array of filled pixels
    - find connected filled components (strokes / outer contours)
    - find background components that are NOT connected to the outside (holes)
    - extract ordered boundary pixels for each contour (sort by angle around centroid)
    - distribute SAMPLE_COUNT samples across contours proportional to their perimeter
    - compute normals per-contour using appropriate centroid rules (holes get inverted normals)
    """
    ...
    contours = []  # entries: {'points':[(x,y)...], 'is_hole':bool, 'centroid':(cx,cy)}
    def boundary_of_component(pixel_coords, interior=True):
        # interior=True -> component is filled; boundary pixels are those with any 4-neighbor not filled
        s = set(pixel_coords)
        b = []
        ...
        if not b:
            return []
        # compute centroid and sort by angle for coarse ordering
    ...
    # collect hole contours (their boundary is where neighbor is filled)
    for hole in holes:
        # boundary of hole: those hole pixels adjacent to filled pixels
        b = []
        for (x, y) in hole:
            for nx, ny in ((x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)):
    ...
    # Compute overall outer centroid (use outer contours only)
    ...
    if outer_pts_flat:
        ...
        # fallback to whole-surface centroid
        letter_centroid = (WIDTH // 2, HEIGHT // 2)

    # distribute SAMPLE_COUNT points across contours proportional to their perimeter
    ...
    norms_out = []
    outer_only = []
    # offset rect to center glyph in window
